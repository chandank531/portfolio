...
import { getGlobalViews } from "../lib/api";
...
export default function RootClient({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetchViews();
  }, [pathname]); // increments global count whenever route changes
      ...
      <footer className="py-8 mt-12 border-t border-gray-200 dark:border-gray-700">
        <div className="max-w-6xl mx-auto flex flex-col items-center space-y-4">
          ...
          <div className="bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-xl font-mono text-blue-600 dark:text-blue-400">
            Total Views:{" "}
            {globalViews !== null ? globalViews : <span>Loading...</span>}
          </div>

          <p className="text-sm text-gray-600 dark:text-gray-400">
            © {new Date().getFullYear()} Kumar Chandan · Built with Next.js &
            Tailwind
          </p>
        </div>
      </footer>
